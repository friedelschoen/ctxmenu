...
// many desktop shell components, and a broad number of other applications
// that interact with the desktop.
func NewLayerShell(ctx *runtime.Context) *LayerShell {
	i := &LayerShell{}
	i.ctx = ctx
	i.id = ctx.Register(i)
	return i
}
func (i *LayerShell) Context() *runtime.Context {
	return i.ctx
}
func (i *LayerShell) ID() uint32 {
	return i.id
}
...
func (i *LayerShell) GetLayerSurface(id *LayerSurface, surface *WlSurface, output *Output, layer uint32, namespace string) error {
	const opcode = 0
...
func (i *LayerShell) Dispatch(opcode uint32, fd int, data []byte) runtime.Event {
...
type LayerSurface struct {
	...
	OnClosed    runtime.EventHandlerFunc
}
...
// The client can re-map the surface by performing a commit without any
// buffer attached, waiting for a configure event and handling it as usual.
func NewLayerSurface(ctx *runtime.Context) *LayerSurface {
	i := &LayerSurface{}
	i.ctx = ctx
	i.id = ctx.Register(i)
	var mu sync.Mutex
...
func (i *LayerSurface) Context() *runtime.Context {
	return i.ctx
}
func (i *LayerSurface) ID() uint32 {
	return i.id
}
func (i *LayerSurface) BlockEvent(e runtime.Event) bool {
...
func (i *LayerSurface) SetSize(width uint32, height uint32) error {
	const opcode = 0
...
func (i *LayerSurface) SetMargin(top int32, right int32, bottom int32, left int32) error {
	const opcode = 3
...
func (i *LayerSurface) GetPopup(popup *Popup) error {
	const opcode = 5
	const _reqBufLen = 8 + 4
	var _reqBuf [_reqBufLen]byte
	l := 0
...
func (i *LayerSurface) SetExclusiveEdge(edge uint32) error {
	const opcode = 9
	const _reqBufLen = 8 + 4
	var _reqBuf [_reqBufLen]byte
	l := 0
...
// WaitForConfigure: waits for LayerSurfaceConfigureEvent
func (i *LayerSurface) WaitForConfigure() LayerSurfaceConfigureEvent {
	for {
		for idx, evt := range i.eventQueue {
			if e, ok := evt.(LayerSurfaceConfigureEvent); ok {
...
// WaitForClosed: waits for LayerSurfaceClosedEvent
func (i *LayerSurface) WaitForClosed() LayerSurfaceClosedEvent {
	for {
		for idx, evt := range i.eventQueue {
			if e, ok := evt.(LayerSurfaceClosedEvent); ok {
				i.eventQueue = slices.Delete(i.eventQueue, idx, idx)
				return e
			}
		}
...
func (i *LayerSurface) Dispatch(opcode uint32, fd int, data []byte) runtime.Event {
	switch opcode {
	case 0:

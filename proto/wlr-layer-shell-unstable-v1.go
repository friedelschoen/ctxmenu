...
// XML file : proto/wlr-layer-shell-unstable-v1.xml
//
...
// Package proto contains wayland-protocol wlr_layer_shell_unstable_v1
package proto
...
// many desktop shell components, and a broad number of other applications
// that interact with the desktop.
func NewLayerShell() *LayerShell {
	return &LayerShell{}
}
func (i *LayerShell) Name() string {
...
func (i *LayerShell) Register(conn *wayland.Conn, id uint32) {
	i.BaseProxy.Register(conn, id)
	if conn != nil && id != 0 {
		runtime.AddCleanup(i, destroyLayerShell, i.BaseProxy)
	}
}
...
func (i *LayerShell) Dispatch(msg *wayland.Message, drain chan<- wayland.Event) {
}
...
type LayerSurfaceHandlers struct {
	...
	OnClosed    wayland.EventHandlerFunc
}
...
// The client can re-map the surface by performing a commit without any
// buffer attached, waiting for a configure event and handling it as usual.
func NewLayerSurface(handlers *LayerSurfaceHandlers) *LayerSurface {
	return &LayerSurface{Handlers: handlers}
}
func (i *LayerSurface) Name() string {
	return "zwlr_layer_surface_v1"
}
...
func (i *LayerSurface) Destroy() {
	destroyLayerSurface(i.BaseProxy)
}
...
func (i *LayerSurface) Register(conn *wayland.Conn, id uint32) {
	i.BaseProxy.Register(conn, id)
	if conn != nil && id != 0 {
		runtime.AddCleanup(i, destroyLayerSurface, i.BaseProxy)
	}
}
...
const (
	...
	// LayerSurfaceErrorInvalidExclusiveEdge : exclusive edge is invalid given the surface anchors
	LayerSurfaceErrorInvalidExclusiveEdge LayerSurfaceError = 4
)
...
func (e LayerSurfaceError) Name() string {
	...
	case LayerSurfaceErrorInvalidExclusiveEdge:
...
func (e *LayerSurfaceConfigureEvent) Proxy() wayland.Proxy {
	return e.proxy
}
...
func (e *LayerSurfaceClosedEvent) Proxy() wayland.Proxy {
	return e.proxy
}
func (i *LayerSurface) Dispatch(msg *wayland.Message, drain chan<- wayland.Event) {
	switch msg.Opcode {
	case 0:
	...
	case 1:
		...
		e.proxy = i
		if i.Handlers != nil && i.Handlers.OnClosed != nil && i.Handlers.OnClosed(e) {
			return
		}
		if drain != nil {
			drain <- e
...
func GetWlrLayerShellUnstableInterface(name string) wayland.Proxy {
	switch name {
	case "zwlr_layer_shell_v1":
		return NewLayerShell()
	case "zwlr_layer_surface_v1":
		return NewLayerSurface(nil)
	default:
		return nil
	}
}

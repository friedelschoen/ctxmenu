...
func (app *Window) attachPointer() {
	...
	app.pointer.SetEnterHandler(app.HandlePointerEnter)
	app.pointer.SetLeaveHandler(app.HandlePointerLeave)
	app.pointer.SetMotionHandler(app.HandlePointerMotion)
	app.pointer.SetButtonHandler(app.HandlePointerButton)
	app.pointer.SetAxisHandler(app.HandlePointerAxis)
	app.pointer.SetAxisSourceHandler(app.HandlePointerAxisSource)
	app.pointer.SetAxisStopHandler(app.HandlePointerAxisStop)
	app.pointer.SetAxisDiscreteHandler(app.HandlePointerAxisDiscrete)
	app.pointer.SetFrameHandler(app.HandlePointerFrame)

	log.Println("pointer interface registered")
}
...
func (app *Window) releasePointer() {
	...
	log.Println("pointer interface released")
}

func (app *Window) HandlePointerEnter(e client.PointerEnterEvent) {
...
func (app *Window) HandlePointerLeave(e client.PointerLeaveEvent) {
	app.pointerEvent.eventMask |= pointerEventLeave
	app.pointerEvent.serial = e.Serial
}

func (app *Window) HandlePointerMotion(e client.PointerMotionEvent) {
...
func (app *Window) HandlePointerButton(e client.PointerButtonEvent) {
...
func (app *Window) HandlePointerAxis(e client.PointerAxisEvent) {
...
func (app *Window) HandlePointerAxisSource(e client.PointerAxisSourceEvent) {
	app.pointerEvent.eventMask |= pointerEventAxis
	app.pointerEvent.axisSource = e.AxisSource
}

func (app *Window) HandlePointerAxisStop(e client.PointerAxisStopEvent) {
	app.pointerEvent.eventMask |= pointerEventAxisStop
	app.pointerEvent.time = e.Time
	app.pointerEvent.axes[e.Axis].valid = true
}

func (app *Window) HandlePointerAxisDiscrete(e client.PointerAxisDiscreteEvent) {
...
func (app *Window) HandlePointerFrame(_ client.PointerFrameEvent) {
	e := app.pointerEvent

	if (e.eventMask & pointerEventEnter) != 0 {
		log.Printf("entered %v, %v", e.surfaceX, e.surfaceY)

		edge := componentEdge(uint32(app.Frame.Rect.Dx()), uint32(app.Frame.Rect.Dy()), e.surfaceX, e.surfaceY, 8)
		log.Println(edge.Name())
	...
	if (e.eventMask & pointerEventLeave) != 0 {
		...
		if err := app.pointer.SetCursor(e.serial, nil, 0, 0); err != nil {
			log.Println("unable to set cursor")
		}
	}
...
func (c *cursorData) Destory() {
	...
	log.Println("destroyed wl_surface for cursor: ", c.name)
}

func (app *Window) setCursor(serial uint32, cursorName string) {
	...
	log.Println("created new wl_surface for cursor: ", cursorName)
